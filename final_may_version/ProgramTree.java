...
public class ProgramTree {
	...
	public ProgramTree(ProgramTree another){
		...
		ProgramTree.fitness = another.fitness;
	}
	...
	public ProgramNode generateRandomNewTree() {
		...
		root = new ProgramNode(type, null, kids, null, float_null, root_depth, ProgramNode.ReturnType.N);
	...
	//for making subtrees based on a given LEAF node 
	...
	public static ProgramNode[] addChildren(float depth, ProgramNode.NodeType node_type, ProgramNode par_node){
		...
		ProgramNode[] kids = new ProgramNode[0];		//what will actually be returned
		...
		switch(node_type) {
			...
			case ASSIGN:
				...
				ProgramNode[] no_kids = new ProgramNode[0];
				kids[0] =  new ProgramNode(ProgramNode.NodeType.VAR, par_node, no_kids, var_name, float_null, actual_depth, ProgramNode.ReturnType.F);
			...
			case GEQ:
				...
				if(kids[0].nodeType == ProgramNode.NodeType.VAR){
					...
					rand_index = randomVal(0, var_names.length);
					var_name = var_names[rand_index];
					ProgramNode[] no_kids1 = new ProgramNode[0];
					kids[1] = new ProgramNode(ProgramNode.NodeType.VAR, par_node, no_kids1, var_name, float_null, actual_depth, ProgramNode.ReturnType.F);
				}
				break;	
	...
	public List<ProgramNode> removeSubtreeFromList(ProgramNode removed_node){
		...
		for(int i = 0; i < num_kids; i++){
			List<ProgramNode> removed = removeSubtreeFromList(kids[i]);
			rm_nodes.addAll(removed); // I THINK THIS COMBINES 2 ARRAYLISTS SMOOTHLY
	...
	public void build_a_tree(){
		System.out.println("number of nodes: " + node_list.size());
